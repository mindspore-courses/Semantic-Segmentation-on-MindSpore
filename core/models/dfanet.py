...
class DFANet(nn.Cell):
    ...
    def construct(self, x):
        ...
        stage1_out = ops.interpolate(stage1_fca, scale_factor=4, mode='bilinear', align_corners=True)

        # stage2
        stage2_enc2 = self.enc2_2(ops.cat([stage1_enc2, stage1_out], axis=1))
        stage2_enc3 = self.enc3_2(ops.cat([stage1_enc3, stage2_enc2], axis=1))
        stage2_enc4 = self.enc4_2(ops.cat([stage1_enc4, stage2_enc3], axis=1))
        stage2_fca = self.fca_2(stage2_enc4)
        stage2_out = ops.interpolate(stage2_fca, scale_factor=4, mode='bilinear', align_corners=True)

        # stage3
        stage3_enc2 = self.enc2_3(ops.cat([stage2_enc2, stage2_out], axis=1))
        stage3_enc3 = self.enc3_3(ops.cat([stage2_enc3, stage3_enc2], axis=1))
        stage3_enc4 = self.enc3_4(ops.cat([stage2_enc4, stage3_enc3], axis=1))
        stage3_fca = self.fca_3(stage3_enc4)

        stage1_enc2_decoder = self.enc2_1_reduce(stage1_enc2)
        stage2_enc2_docoder = ops.interpolate(self.enc2_2_reduce(stage2_enc2), scale_factor=2,
                                            mode='bilinear', align_corners=True)
        stage3_enc2_decoder = ops.interpolate(self.enc2_3_reduce(stage3_enc2), scale_factor=4,
                                            mode='bilinear', align_corners=True)
        fusion = stage1_enc2_decoder + stage2_enc2_docoder + stage3_enc2_decoder
        fusion = self.conv_fusion(fusion)

        stage1_fca_decoder = ops.interpolate(self.fca_1_reduce(stage1_fca), scale_factor=4,
                                           mode='bilinear', align_corners=True)
        stage2_fca_decoder = ops.interpolate(self.fca_2_reduce(stage2_fca), scale_factor=8,
                                           mode='bilinear', align_corners=True)
        stage3_fca_decoder = ops.interpolate(self.fca_3_reduce(stage3_fca), scale_factor=16,
                                           mode='bilinear', align_corners=True)
        fusion = fusion + stage1_fca_decoder + stage2_fca_decoder + stage3_fca_decoder

        outputs = list()
        out = self.conv_out(fusion)
        out = ops.interpolate(out, scale_factor=4, mode='bilinear', align_corners=True)
        outputs.append(out)
