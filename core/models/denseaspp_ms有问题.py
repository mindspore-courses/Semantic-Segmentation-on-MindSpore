...
class DenseASPP(nn.Cell):
    ...
    def construct(self, x):
        ...
        if self.dilate_scale > 8:
            features = P.interpolate(features, scale_factor=2, mode='bilinear', align_corners=True)
        outputs = []
...
class _DenseASPPConv(nn.SequentialCell):
    def __init__(self, in_channels, inter_channels, out_channels, atrous_rate,
        ...
        self.add_module('conv2', nn.Conv2d(inter_channels, out_channels, 3, dilation=atrous_rate, pad_mode='pad', padding=atrous_rate)),
    ...
    def construct(self, x):
        ...
        if self.drop_rate > 0:
            features = P.dropout(features, p=self.drop_rate, training=self.training)
        return features


class _DenseASPPBlock(nn.Cell):
    ...
    def construct(self, x):
        aspp3 = self.aspp_3(x)
        x = P.cat([aspp3, x], axis=1)

        aspp6 = self.aspp_6(x)
        x = P.cat([aspp6, x], axis=1)

        aspp12 = self.aspp_12(x)
        x = P.cat([aspp12, x], axis=1)

        aspp18 = self.aspp_18(x)
        x = P.cat([aspp18, x], axis=1)

        aspp24 = self.aspp_24(x)
        x = P.cat([aspp24, x], axis=1)
