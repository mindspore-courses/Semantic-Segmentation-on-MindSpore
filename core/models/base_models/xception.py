...
class SeparableConv2d(nn.Cell):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, dilation=1, bias=False):
        ...
        self.conv1 = nn.Conv2d(in_channels, in_channels, kernel_size, stride, pad_mode='pad', padding=0, dilation=dilation, group=in_channels,
                               has_bias=bias)
        self.bn = nn.BatchNorm2d(in_channels)
        self.pointwise = nn.Conv2d(in_channels, out_channels, 1, has_bias=bias)
    ...
    def fix_padding(self, x, kernel_size, dilation):
        ...
        padded_inputs = P.Pad(((0,0), (0,0), (pad_beg, pad_end), (pad_beg, pad_end)))(x)
        return padded_inputs

class Block(nn.Cell):
    def __init__(self, in_channels, out_channels, reps, stride=1, dilation=1, start_with_relu=True, grow_first=True, is_last=False):
        super(Block, self).__init__()
        if out_channels != in_channels or stride != 1:
            self.skip = nn.Conv2d(in_channels, out_channels, 1, stride, has_bias=False)
        ...
        if stride != 1:
            ...
            rep.append(self.relu)
            rep.append(SeparableConv2d(out_channels, out_channels, 3, 1, dilation))
            rep.append(nn.BatchNorm2d(out_channels))
...
# Xception65, Xception71, get_xception, get_xception_71等类和函数的实现方式类似，这里省略
class Xception65(nn.Cell):
    ...
    def __init__(self, num_classes=1000, output_stride=32, norm_layer=nn.BatchNorm2d):
        ...
        # Entry flow
        self.conv1 = nn.Conv2d(3, 32, 3, 2, pad_mode='pad', padding=1, has_bias=False)
        self.bn1 = norm_layer(32)
        self.relu = nn.ReLU()

        self.conv2 = nn.Conv2d(32, 64, 3, 1, pad_mode='pad', padding=1, has_bias=False)
        ...
        midflow = list()
        for i in range(4, 20):
            midflow.append(Block(728, 728, reps=3, stride=1, dilation=middle_block_dilation,
                                 start_with_relu=True, grow_first=True))
        self.midflow = nn.SequentialCell(*midflow)
...
class Xception71(nn.Cell):
    ...
    def __init__(self, num_classes=1000, output_stride=32, norm_layer=nn.BatchNorm2d):
        ...
        self.block2 = nn.SequentialCell(
            ...
            Block(256, 728, reps=2, stride=2, start_with_relu=False, grow_first=True))
...
#                   For DFANet
# -------------------------------------------------
class BlockA(nn.Cell):
    def __init__(self, in_channels, out_channels, stride=1, dilation=1, start_with_relu=True):
        ...
        if start_with_relu:
            rep.append(self.relu)
        rep.append(SeparableConv2d(in_channels, inter_channels, 3, 1, dilation))
        rep.append(nn.BatchNorm2d(inter_channels))

        rep.append(self.relu)
        rep.append(SeparableConv2d(inter_channels, inter_channels, 3, 1, dilation))
        rep.append(nn.BatchNorm2d(inter_channels))
...
class FCAttention(nn.Cell):
    def __init__(self, in_channels, norm_layer=nn.BatchNorm2d):
        super(FCAttention, self).__init__()
        self.avgpool = nn.AdaptiveAvgPool2d(1)
        self.fc = nn.Dense(in_channels, 1000)
        self.conv = nn.SequentialCell(
            nn.Conv2d(1000, in_channels, 1, has_bias=False),
...
class XceptionA(nn.Cell):
    def __init__(self, num_classes=1000, norm_layer=nn.BatchNorm2d):
        super(XceptionA, self).__init__()
        self.conv1 = nn.SequentialCell(nn.Conv2d(3, 8, 3, 2, pad_mode='pad', padding=1, has_bias=False),
                                    norm_layer(8),
                                    nn.ReLU())
        ...
        self.avgpool = nn.AdaptiveAvgPool2d(1)
        self.fc = nn.Dense(192, num_classes)
...
def get_xception(pretrained=False, root='~/.mindspore/models', **kwargs):
    ...
    if pretrained:
        ...
        param_dict = load_checkpoint(get_model_file('xception', root=root))
        load_param_into_net(model, param_dict)
    return model


def get_xception_71(pretrained=False, root='~/.mindspore/models', **kwargs):
    model = Xception71(**kwargs)
    if pretrained:
        from ..model_store import get_model_file
        param_dict = load_checkpoint(get_model_file('xception71', root=root))
        load_param_into_net(model, param_dict)
    return model


def get_xception_a(pretrained=False, root='~/.mindspore/models', **kwargs):
    model = XceptionA(**kwargs)
    if pretrained:
        from ..model_store import get_model_file
        param_dict = load_checkpoint(get_model_file('xception_a', root=root))
        load_param_into_net(model, param_dict)
    return model
