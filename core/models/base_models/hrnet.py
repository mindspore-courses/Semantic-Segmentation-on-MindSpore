...
class BasicBlock(nn.Cell):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, norm_layer=nn.BatchNorm2d):
        ...
        self.conv1 = nn.Conv2d(inplanes, planes, 3, stride, pad_mode='pad', padding=1, has_bias=False)
...
class Bottleneck(nn.Cell):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, norm_layer=nn.BatchNorm2d):
        ...
        self.bn1 = norm_layer(planes)
        self.conv2 = nn.Conv2d(planes, planes, 3, stride, pad_mode='pad', padding=1, has_bias=False)
        self.bn2 = norm_layer(planes)
            ...
            for j in range(num_branches):
                if j > i:
                    fuse_layer.append(nn.SequentialCell(
                        nn.Conv2d(num_inchannels[j], num_inchannels[i], 1, has_bias=False),
                    ...
                    for k in range(i - j):
                        if k == i - j - 1:
                            ...
                            conv3x3s.append(nn.SequentialCell(
                                nn.Conv2d(num_inchannels[j], num_outchannels_conv3x3, 3, 2,pad_mode='pad', padding=1, has_bias=False),
...
class HighResolutionNet(nn.Cell):
    def __init__(self, blocks, num_channels, num_modules, num_branches, num_blocks,
        ...
        self.conv1 = nn.SequentialCell(
            ...
            norm_layer(64),
            nn.ReLU(),
            nn.Conv2d(64, 64, 3, 2, pad_mode='pad', padding=1, has_bias=False),
            norm_layer(64),
            nn.ReLU())
    ...
    def _make_layer(self, block, inplanes, planes, blocks, stride=1, norm_layer=nn.BatchNorm2d):
        ...
        if stride != 1 or inplanes != planes * block.expansion:
            downsample = nn.SequentialCell(
                nn.Conv2d(inplanes, planes * block.expansion, 1, stride, has_bias=False),
            ...
            if i < num_branches_pre:
                if num_channels_cur_layer[i] != num_channels_pre_layer[i]:
                    transition_layers.append(nn.SequentialCell(
                        nn.Conv2d(num_channels_pre_layer[i], num_channels_cur_layer[i], 3, 1, pad_mode='pad', padding=1, has_bias=False),
                        norm_layer(num_channels_cur_layer[i]),
                        nn.ReLU()))
                else:
                    transition_layers.append(nn.Cell())  # 使用空的Cell代替None
            else:
                conv3x3s = list()
                for j in range(i + 1 - num_branches_pre):
                    ...
                    conv3x3s.append(nn.SequentialCell(
                        nn.Conv2d(in_channels, out_channels, 3, 2, pad_mode='pad', padding=1, has_bias=False),
    ...
    def construct(self, x):
        ...
        y = ops.avg_pool2d(y, kernel_size=y.size()
        [2:]).ops.reshape(x, (x.size(0), -1))
