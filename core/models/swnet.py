...
#block 6:
        self.fullconv = nn.Conv2dTranspose(16, nclass, 2, 2, has_bias=False)
...
class InitialBlock(nn.Cell):
    ...
    def __init__(self, out_channels, norm_layer=nn.BatchNorm2d, **kwargs):
        ...
        self.conv = nn.Conv2d(3, out_channels, 3, 2, pad_mode='pad', padding=1, has_bias=False)
    ...
    def construct(self, x):
        ...
        x = ops.cat([x_conv, x_pool], axis=1)
        x = self.bn(x)
        x = self.act(x)
        return x
...
class Bottleneck(nn.Cell):
    ...
    def __init__(self, in_channels, inter_channels, out_channels, dilation=1, asymmetric=False,
        ...
        if downsampling:
            self.maxpool = nn.MaxPool2d(2, 2, pad_mode='pad', return_indices=True)
            self.conv_down = nn.SequentialCell(
                nn.Conv2d(in_channels, out_channels, 1, has_bias=False),
                norm_layer(out_channels)
            )

        self.conv1 = nn.SequentialCell(
            nn.Conv2d(in_channels, inter_channels, 1, has_bias=False),
        ...
        if downsampling:
            self.conv2 = nn.SequentialCell(
                nn.Conv2d(inter_channels, inter_channels, 2, stride=2, has_bias=False),
                norm_layer(inter_channels),
                nn.RReLU()
            )
        else:
            if asymmetric:
                self.conv2 = nn.SequentialCell(
                    nn.Conv2d(inter_channels, inter_channels, (5, 1), pad_mode='pad', padding=(2, 0, 2, 0), has_bias=False),
                    nn.Conv2d(inter_channels, inter_channels, (1, 5), pad_mode='pad', padding=(0, 2, 0, 2), has_bias=False),
                    norm_layer(inter_channels),
                    nn.RReLU()
                )
            else:
                self.conv2 = nn.SequentialCell(
                    nn.Conv2d(inter_channels, inter_channels, 3, dilation=dilation, pad_mode='pad', padding=dilation, has_bias=False),
                    norm_layer(inter_channels),
                    nn.RReLU()
                )
...
class UpsamplingBottleneck(nn.Cell):
    """upsampling Block"""
    ...
    def __init__(self, in_channels, inter_channels, out_channels, norm_layer=nn.BatchNorm2d, **kwargs):
        ...
        self.block = nn.SequentialCell(
            ...
            norm_layer(inter_channels),
            nn.RReLU(),
            nn.Conv2dTranspose(inter_channels, inter_channels, 2, 2, has_bias=False),
            norm_layer(inter_channels),
            nn.RReLU(),
            nn.Conv2d(inter_channels, out_channels, 1, has_bias=False),
            norm_layer(out_channels),
    ...
    def construct(self, x, max_indices):
        out_up = self.conv(x)
