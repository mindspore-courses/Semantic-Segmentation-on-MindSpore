...
class ICNet(SegBaseModel):
    ...
    def construct(self, x):
        ...
        # sub 2
        x_sub2 = ops.interpolate(x, scale_factor=0.5, mode='nearest', align_corners=True)
        _, x_sub2, _, _ = self.base_forward(x_sub2)

        # sub 4
        x_sub4 = ops.interpolate(x, scale_factor=0.25, mode='bilinear', align_corners=True)
...
class PyramidPoolingModule(nn.Cell):
    ...
    def construct(self, input):
        ...
        for bin_size in self.pyramids:
            x = ops.adaptive_avg_pool2d(input, output_size=bin_size)
            x = ops.interpolate(x, size=(height, width), mode='bilinear', align_corners=True)
            feat = feat + x
        return feat
...
class _ICHead(nn.Cell):
    ...
    def construct(self, x_sub1, x_sub2, x_sub4):
        ...
        up_x2 = ops.interpolate(x_cff_12, scale_factor=2, mode='bilinear', align_corners=True)
        up_x2 = self.conv_cls(up_x2)
        outputs.append(up_x2)
        up_x8 = ops.interpolate(up_x2, scale_factor=4, mode='bilinear', align_corners=True)
...
class _ConvBNReLU(nn.Cell):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, dilation=1,
        ...
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size, stride,pad_mode='pad', padding=padding, dilation=dilation, group=groups, has_bias=bias)
...
class CascadeFeatureFusion(nn.Cell):
    """CFF Unit"""
    ...
    def __init__(self, low_channels, high_channels, out_channels, nclass, norm_layer=nn.BatchNorm2d, **kwargs):
        ...
        self.conv_high = nn.SequentialCell(
            nn.Conv2d(high_channels, out_channels, 1, has_bias=False),
            norm_layer(out_channels)
        )
        self.conv_low_cls = nn.Conv2d(out_channels, nclass, 1, has_bias=False)

    def construct(self, x_low, x_high):
        x_low = ops.interpolate(x_low, size=x_high.size()[2:], mode='bilinear', align_corners=True)
