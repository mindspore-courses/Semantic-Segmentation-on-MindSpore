...
class _AsppPooling(nn.Cell):
    ...
    def construct(self, x):
        size = x.shape[2:]
        pool = self.gap(x)
        out = P.interpolate(pool, size, mode='bilinear', align_corners=True)
        return out
...
class _ASPP(nn.Cell):
    def __init__(self, in_channels, atrous_rates, norm_layer, norm_kwargs, **kwargs):
        ...
        self.project = nn.SequentialCell(
            nn.Conv2d(5 * out_channels, out_channels, 1, has_bias=False),
            norm_layer(out_channels, **({} if norm_kwargs is None else norm_kwargs)),
            nn.ReLU(),
            nn.Dropout(p=0.5)
        )
    ...
    def construct(self, x):
        ...
        feat4 = self.b3(x)
        feat5 = self.b4(x)
        x = P.cat((feat1, feat2, feat3, feat4, feat5), axis=1)
        x = self.project(x)
        return x
