...
class CGNet(nn.Cell):
    ...
    def construct(self, x):
        ...
        # stage 2
        out0_cat = self.bn_prelu1(ops.cat([out0, inp1], axis=1))
        ...
        for i, layer in enumerate(self.stage2):
            if i == 0:
                ...
                out1 = layer(out1)
        out1_cat = self.bn_prelu2(ops.cat([out1, out1_0, inp2], axis=1))
        ...
        for i, layer in enumerate(self.stage3):
            if i == 0:
                ...
                out2 = layer(out2)
        out2_cat = self.bn_prelu3(ops.cat([out2_0, out2], axis=1))
...
class _ChannelWiseConv(nn.Cell):
    def __init__(self, in_channels, out_channels, dilation=1, **kwargs):
        super(_ChannelWiseConv, self).__init__()
        self.conv = nn.Conv2d(in_channels, out_channels, 3, 1, pad_mode='pad', padding=dilation, dilation=dilation, group=in_channels, has_bias=False)
...
class _FGlo(nn.Cell):
    def __init__(self, in_channels, reduction=16, **kwargs):
        ...
        self.fc = nn.SequentialCell(
            nn.Dense(in_channels, in_channels // reduction),
            nn.ReLU(),
            nn.Dense(in_channels // reduction, in_channels),
            nn.Sigmoid())
...
class _InputInjection(nn.Cell):
    def __init__(self, ratio):
        super(_InputInjection, self).__init__()
        self.pool = nn.CellList()
        for i in range(0, ratio):
            self.pool.append(nn.AvgPool2d(3, 2, pad_mode='pad', padding=1))
...
class _ConcatInjection(nn.Cell):
...
class ContextGuidedBlock(nn.Cell):
    def __init__(self, in_channels, out_channels, dilation=2, reduction=16, down=False,
        ...
        if down:
            ...
            self.reduce = nn.Conv2d(inter_channels * 2, out_channels, 1, has_bias=False)
        else:
    ...
    def construct(self, x):
        ...
        joi_feat = ops.cat([loc, sur], axis=1)
