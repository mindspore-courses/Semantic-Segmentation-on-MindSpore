...
class SeparableConv2d(nn.Cell):
    def __init__(self, inplanes, planes, kernel_size=3, stride=1, pad_mode='pad', padding=1,
                 dilation=1, bias=False, norm_layer=nn.BatchNorm2d):
        super(SeparableConv2d, self).__init__()
        self.conv = nn.Conv2d(inplanes, inplanes, kernel_size, stride,pad_mode='pad', padding=padding, dilation=dilation, group=inplanes, has_bias=bias)
...
class JPU(nn.Cell):
    def __init__(self, in_channels, width=512, norm_layer=nn.BatchNorm2d, **kwargs):
        ...
        self.dilation1 = nn.SequentialCell(
            SeparableConv2d(3 * width, width, 3, pad_mode='pad', padding=1, dilation=1, bias=False),
            norm_layer(width),
            nn.ReLU())
        self.dilation2 = nn.SequentialCell(
            SeparableConv2d(3 * width, width, 3, pad_mode='pad', padding=2, dilation=2, bias=False),
            norm_layer(width),
            nn.ReLU())
        self.dilation3 = nn.SequentialCell(
            SeparableConv2d(3 * width, width, 3, pad_mode='pad', padding=4, dilation=4, bias=False),
            norm_layer(width),
            nn.ReLU())
        self.dilation4 = nn.SequentialCell(
            SeparableConv2d(3 * width, width, 3, pad_mode='pad', padding=8, dilation=8, bias=False),
            norm_layer(width),
            nn.ReLU())
